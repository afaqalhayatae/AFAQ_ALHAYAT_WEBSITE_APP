...
import PestControlCityPage, { generateStaticParams } from "./page";
 ...
 * fixture) lives on the maintenance section-city route.
 */
describe("PestControlCityPage", () => {
  ...
  it("404s for any city", async () => {
    await expect(
      PestControlCityPage({ params: Promise.resolve({ locale: "en", city: "dubai" }) })
    ).rejects.toThrow();
  });

  it("404s for an unsupported locale", async () => {
    await expect(
      PestControlCityPage({ params: Promise.resolve({ locale: "fr", city: "dubai" }) })
    ).rejects.toThrow();
  });
});
